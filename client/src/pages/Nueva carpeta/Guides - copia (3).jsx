...
const Guides = () => {
  ...
  const printFilteredGuides = async () => {
    try {
      ...
      // Obtener todas las guías filtradas
      ...
      // ✅ CONFIGURACIÓN DE LOTES (IGUAL QUE BULK UPLOAD)
      const batchSize = 102; // ✅ MISMO TAMAÑO QUE EL MÓDULO DE CARGA MASIVA
      const totalBatches = Math.ceil(totalGuides / batchSize);
      
      // Confirmar con el usuario
      const confirm = window.confirm(
        `⚠️ ADVERTENCIA: Vas a imprimir ${totalGuides} guías.\n\n` +
        `• Se generarán ${totalBatches} archivos PDF\n` +
      ...
      // ✅ PROCESAR EN LOTES DE 102 (MISMA LÓGICA QUE generatePDF)
      for (let i = 0; i < allGuideIds.length; i += batchSize) {
        const batch = allGuideIds.slice(i, i + batchSize);
        const batchNumber = Math.floor(i / batchSize) + 1;
        
        const toastId = toast.loading(`Generando PDF ${batchNumber} de ${totalBatches}...`);
        
        try {
          // ✅ REUTILIZAR downloadPDF existente (mismo formato que bulk)
          ...
          toast.dismiss(toastId);
          toast.success(`✅ Lote ${batchNumber}: ${batch.length} guías descargadas`);
          
          // ✅ Esperar 1 segundo entre lotes para no saturar el servidor
          ...
          toast.dismiss(toastId);
          toast.error(`❌ Error en el lote ${batchNumber}`);
          console.error(`Error en lote ${batchNumber}:`, error);
        }
      }
      
      toast.success(`🎉 Proceso completado: ${totalBatches} PDFs generados`);
