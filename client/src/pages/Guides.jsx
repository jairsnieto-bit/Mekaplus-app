...
import GuideEvidenceModal from '../components/GuideEvidenceModal';  // ✅ NUEVO
...
const Guides = () => {
  ...
  const [showEvidenceModal, setShowEvidenceModal] = useState(false);  // ✅ NUEVO
  ...
  const fetchAllFilteredGuides = async () => {
    try {
      ...
      const response = await guideAPI.getAll({ 
        ...params, 
        page: 1, 
        limit: 100000  // ✅ Límite muy alto para obtener todas
      });
  ...
  // ✅ NUEVA FUNCIÓN: Imprimir TODAS las guías filtradas (EN LOTES DE 102)
  ...
  try {
    ...
    // ✅ MANTENER 102 guías por lote
    const batchSize = 102;  // ✅ 102 guías
    const totalBatches = Math.ceil(totalGuides / batchSize);
    
    const confirm = window.confirm(
      `⚠️ ADVERTENCIA: Vas a imprimir ${totalGuides} guías.\n\n` +
      `• Se generarán ${totalBatches} archivo(s) PDF\n` +
      `• Cada PDF tendrá máximo ${batchSize} guías\n` +
      `• Tamaño estimado: ${(totalGuides * 0.15).toFixed(1)} MB\n` +
      `• Tiempo estimado: ${Math.ceil(totalGuides / 10)} segundos\n` +
      `• ¿Deseas continuar?`
    );
    ...
    for (let i = 0; i < allGuideIds.length; i += batchSize) {
      const batch = allGuideIds.slice(i, i + batchSize);
      const batchNumber = Math.floor(i / batchSize) + 1;
      
      const toastId = toast.loading(
        `📄 Generando PDF ${batchNumber} de ${totalBatches}...\n` +
        `(${batch.length} guías)`
      );
      
      try {
        console.log(`📤 Enviando lote ${batchNumber}: ${batch.length} guías`);
        
        const response = await guideAPI.downloadPDF(batch);
        
        const blob = new Blob([response.data], { type: 'application/pdf' });
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `guias-filtradas-lote-${batchNumber}-${Date.now()}.pdf`;
        link.click();
        window.URL.revokeObjectURL(url);
        ...
        try {
          const retryResponse = await guideAPI.downloadPDF(batch);
          // ... mismo código de descarga
          toast.success(`✅ Lote ${batchNumber} reintentado exitosamente`);
        } catch (retryError) {
          toast.error(`❌ Lote ${batchNumber} falló definitivamente`);
        }
      }
    }
    
    toast.success(`🎉 Proceso completado:\n${totalBatches} PDF(s) generados`);
    
  } catch (error) {
    toast.error('Error al imprimir guías filtradas');
    console.error('Error en printFilteredGuides:', error);
  } finally {
    setPrintingFiltered(false);
  }
};
  // ✅ NUEVA FUNCIÓN: Exportar guías filtradas a Excel
const exportToExcel = async () => {
  try {
    ...
    if (totalGuides === 0) {
      toast.error('No hay guías para exportar con los filtros actuales');
      return;
    }
    
    const confirm = window.confirm(
      `📊 Exportar a Excel\n\n` +
      `• Se exportarán ${totalGuides} guías\n` +
      `• Formato: Excel (.xlsx)\n` +
      `• ¿Deseas continuar?`
    );
    ...
    try {
      ...
      console.log('📊 [EXPORT] Enviando filtros:', exportFilters);
      
      const response = await guideAPI.exportExcel(exportFilters);
      ...
      link.download = `informe_guias_${timestamp}.xlsx`;
      
      link.click();
      window.URL.revokeObjectURL(url);
      
      toast.dismiss(toastId);
      toast.success(`✅ Informe generado: ${totalGuides} guías exportadas`);
      ...
      if (error.response?.status === 404) {
        ...
        toast.error('Error al generar informe Excel');
      }
  ...
  /*const openStatusModal = (guide) => {
    ...
    setShowStatusModal(true);
      ...
      const openStatusModal = (guide) => {
        console.log('📋 [MODAL] Abriendo modal para guía:', guide);
        console.log('📋 [MODAL] ID de guía:', guide.id);
        console.log('📋 [MODAL] Número de guía:', guide.guideNumber);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            {/* ✅ NUEVO: Botón Generar Informe Excel */}
      <button
        ...
        title="Exportar guías filtradas a Excel"
      >
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left">No. Liquidación</th>
              ...
              {loading ? (
                ...
                guides.map((guide) => (
                  <tr key={guide.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3"> <span className="text-sm text-gray-700">
                        {guide.identificacionUsuario || '-'}</span></td>
      ...
      {showStatusModal && selectedGuide && (
        ...
        {/* ✅ NUEVO: Modal para ver evidencia */}
        {showEvidenceModal && selectedGuide && (
          <GuideEvidenceModal
            guide={selectedGuide}
            onClose={() => {
