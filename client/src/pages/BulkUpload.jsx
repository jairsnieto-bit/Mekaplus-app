  ...
  useEffect(() => {
    const fetchSenders = async () => {
      try {
        ...
        //const statsData = statsResponse.data;  // ✅ ¡Esta línea es obligatoria!
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          // ⚠️ Si son más de 102, dividir en lotes
                          if (totalGuides > 102) {
                            const confirm = window.confirm(
                              `⚠️ ADVERTENCIA: Vas a imprimir ${totalGuides} guías.\n\n` +
                              `• Se generarán ${Math.ceil(totalGuides/102)} archivos PDF\n` +
                              `• Cada PDF tendrá máximo 102 guías\n\n` +
                              `¿Deseas continuar?`
                            );
                            if (!confirm) return;
                            
                            // Descargar en lotes de 100
                            const batchSize = 102;
                            for (let i = 0; i < totalGuides; i += batchSize) {
                              const batch = result.allGuideIds.slice(i, i + batchSize);
                              const batchNumber = Math.floor(i / batchSize) + 1;
                              
                              toast.loading(`Descargando lote ${batchNumber}...`);
                              ...
                              try {
                                ...
                                const link = document.createElement('a');
                                link.href = url;
                                link.download = `guias-masivas-lote-${batchNumber}-${Date.now()}.pdf`;
                                link.click();
                                window.URL.revokeObjectURL(url);
                                
                                toast.dismiss();
                                toast.success(`Lote ${batchNumber} descargado`);
                                
                                // Esperar 1 segundo entre lotes
                                await new Promise(resolve => setTimeout(resolve, 1000));
                              } catch (error) {
                                toast.dismiss();
                                toast.error(`Error en lote ${batchNumber}`);
                              }
                            }
                            
                            toast.success(`Se descargaron ${Math.ceil(totalGuides/batchSize)} lotes`);
                          } else {
                            // Menos de 200 guías - descargar todo junto
                            toast.loading('Generando PDF... Esto puede tardar', { duration: 300000 });
                            
                            const response = await guideAPI.downloadPDF(result.allGuideIds);
                            
                            const blob = new Blob([response.data], { type: 'application/pdf' });
                            const url = window.URL.createObjectURL(blob);
                            const link = document.createElement('a');
                            link.href = url;
                            link.download = `guias-masivas-${totalGuides}-${Date.now()}.pdf`;
                            link.click();
                            window.URL.revokeObjectURL(url);
