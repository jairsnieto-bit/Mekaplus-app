...
class GuideService {
  ...
  async getGuides(filters) {
    try {
      const { page, limit, search, status, city, guideNumber, startDate, endDate } = filters;
      ...
      // ✅ REMOVIDO: include createdByUser (no existe en el schema)
...
async createSender(data) {
  ...
  async deleteGuide(id) {
    ...
    async updateGuideStatus(id, status, observation, userId) {
      try {
        // Validar observación
