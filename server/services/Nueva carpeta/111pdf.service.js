...
class PDFService {
  ...
  // ✅ MÉTODOS AUXILIARES PARA REMITENTE
  getDefaultSender() {
    return {
      name: 'Mekaplus express S.A.S',
      nit: '901505437-1',
      address: 'Principal: Cartagena Colombia Sec Víctor Blanco Mz.173.LT.10',
      phone: '3115922099',
  ...
  // ✅ INFORMACIÓN FIJA PARA SIDEBAR
  getFixedSidebarInfo() {
    return `
      Mekaplus express S.A.S NIT. 901505437-1<br/>
      Principal: Cartagena Colombia Sec Víctor Blanco Mz.173.LT.10<br/>
      <br/>
      REMITE:<br/>
  ...
  async generateGuideHTML(guide, config, settings) {
    ...
    // ✅ NUEVO: Información fija para el sidebar
    const fixedSidebarInfo = this.getFixedSidebarInfo();
      ...
      <body>
        <div class="guide">
          <div class="guide-grid">
            ...
            <!-- Sidebar Vertical con INFO FIJA -->
            ...
            <section class="right-block">
              ...
              <!-- Fecha, Hora, Número de Guía y QR -->
  ...
  async renderPage(guides, config, settings, pageIndex, totalPages) {
    ...
    return `
      <div class="page ${guidesCount === 1 ? 'page-single' : guidesCount === 2 ? 'page-double' : 'page-triple'}">
        ${guides.map(async (guide, index) => {
          const guideHTML = await this.generateGuideHTML(guide, config, settings);
          return `
  ...
  getPrintStyles() {
    return `
      * {
      ...
      /* ✅ NUEVO: Sección QR + Número de Guía */
      .guide-qr-section {
        display: flex;
        flex-direction: column;
