...
async createSender(data) {
  ...
  /*async getGuideById(id) {
    try {
      ...
      throw new Error(`Error al obtener guía: ${error.message}`);
        ...
        async getGuideById(id) {
          try {
            console.log('🔍 [SERVICE] Buscando guía con ID:', id);
            ...
            // ✅ Buscar guía con include de sender
            const guide = await prisma.guide.findUnique({
              where: { id },
              include: {
                sender: true  // ✅ Incluir datos del remitente
              }
            });

            if (!guide) {
              console.warn('⚠️ [SERVICE] Guía no encontrada. ID:', id);
              
              // ✅ Listar IDs existentes para debug (solo en desarrollo)
              if (process.env.NODE_ENV === 'development') {
                const existingGuides = await prisma.guide.findMany({
                  select: { id: true, guideNumber: true },
                  take: 5
                });
                console.log('📋 [DEBUG] Últimas 5 guías existentes:', existingGuides);
              }
              
              throw new Error('Guía no encontrada');
            }

            console.log('✅ [SERVICE] Guía encontrada:', guide.guideNumber);
            return guide;
          } catch (error) {
            console.error('❌ [SERVICE] Error en getGuideById:', error.message);
            throw new Error(`Error al obtener guía: ${error.message}`);
          }
        }
  ...
  try {
    console.log(`📄 [PDF SERVICE] Generando PDF para ${guidesIds.length} guías...`);

    // ✅ Obtener todas las guías
    const guides = await this.getGuidesByIds(guidesIds);
    console.log(`✅ Guías obtenidas: ${guides.length}`);
    ...
    console.log('🖨️ Iniciando generación con Puppeteer...');
    ...
    // ✅ AGREGAR: Obtener guías por IDs (para lotes)
      ...
      /*// ✅ NUEVO: Obtener destinatarios únicos de guías anteriores
        async getExistingRecipients() {
          try {
            // Obtenemos guías recientes para extraer destinatarios únicos
            const guides = await prisma.guide.findMany({
              select: {
                razonSocial: true,
                direccion: true,
                localidad: true,
                identificacionUsuario: true
              },
              orderBy: { createdAt: 'desc' },
              take: 100 // Últimos 100 registros para no saturar
            });

            // Filtrar duplicados en JavaScript
            const uniqueRecipients = [];
            const seen = new Set();

            guides.forEach(guide => {
              const key = `${guide.razonSocial}-${guide.direccion}`;
              if (!seen.has(key) && guide.razonSocial) {
                seen.add(key);
                uniqueRecipients.push({
                  razonSocial: guide.razonSocial,
                  direccion: guide.direccion,
                  localidad: guide.localidad,
                  identificacionUsuario: guide.identificacionUsuario
                });
              }
            });
       ...
       // ✅ NUEVO: Obtener destinatarios únicos de guías anteriores
          async getExistingRecipients() {
            try {
              console.log('📋 [SERVICE] Obteniendo destinatarios existentes...');
              ...
              console.log(`📋 [SERVICE] ${guides.length} guías encontradas`);
              ...
              guides.forEach(guide => {
                // Crear clave única basada en razón social y dirección
                const key = `${guide.razonSocial}-${guide.direccion}`.toLowerCase();
              ...
              console.log(`✅ [SERVICE] ${uniqueRecipients.length} destinatarios únicos encontrados`);
              return uniqueRecipients;
            } catch (error) {
              console.error('❌ [SERVICE] Error en getExistingRecipients:', error);
              throw new Error(`Error al obtener destinatarios: ${error.message}`);
            }
          }
