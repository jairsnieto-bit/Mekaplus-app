...
class GuideController {
  ...
  /*async getGuideById(req, res) {
    ...
    async getGuideById(req, res) {
      try {
        console.log('🔍 [DEBUG] Buscando guía con ID:', req.params.id);
        console.log('🔍 [DEBUG] Tipo de ID:', typeof req.params.id);
        
        const guide = await guideService.getGuideById(req.params.id);
        res.json(guide);
      } catch (error) {
        console.error('❌ [ERROR] No se encontró la guía:', error.message);
        console.error('❌ [ERROR] ID buscado:', req.params.id);
        res.status(404).json({ error: error.message });
      }
      }
...
async updateGuideStatus(req, res) {
  ...
  try {
    ...
    res.json({
      ...
      /*async getExistingRecipients(req, res) {
        try {
          const recipients = await guideService.getExistingRecipients();
          res.json(recipients);
        } catch (error) {
          res.status(500).json({ error: error.message });
        }
        ...
        async getExistingRecipients(req, res) {
            try {
              console.log('📋 [CONTROLLER] Obteniendo destinatarios existentes...');
              const recipients = await guideService.getExistingRecipients();
              console.log(`✅ [CONTROLLER] ${recipients.length} destinatarios encontrados`);
              res.json(recipients);
            } catch (error) {
              console.error('❌ [CONTROLLER] Error obteniendo destinatarios:', error);
              res.status(500).json({ error: error.message });
            }
          }


          // ✅ NUEVO: Exportar guías a Excel
          async exportToExcel(req, res) {
            try {
              const filters = req.body;
              
              console.log('📊 [EXPORT EXCEL] Filtros recibidos:', filters);
              
              // Obtener guías con filtros (sin paginación)
              const allGuides = await guideService.getGuides({
                ...filters,
                page: 1,
                limit: 100000  // Límite alto para obtener todas
              });
              
              const guides = allGuides.guides;
              
              if (guides.length === 0) {
                return res.status(404).json({ error: 'No hay guías para exportar con los filtros aplicados' });
              }
              
              // ✅ Preparar datos para Excel
              const excelData = guides.map(guide => ({
                'Número': guide.guideNumber,
                'No. Liquidación': guide.identificacionUsuario || '-',
                'Razón Social': guide.razonSocial,
                'Dirección': guide.direccion,
                'Ciudad': guide.localidad,
                'Estado o referencia': guide.estado || guide.referenciaEntrega || '-',
                'Fecha': new Date(guide.createdAt).toLocaleDateString('es-CO')
              }));
              ...
              const filename = `informe_guias_${timestamp}.xlsx`;
              
              res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
              res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
              
              console.log(`✅ [EXPORT EXCEL] Archivo generado: ${filename} (${guides.length} guías)`);
